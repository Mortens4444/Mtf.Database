...
if (!BaseRepository.HasValidSqlSyntax("DELETE FROM Permissions WHERE GroupId = @GroupId;", false, out var ex))
{
    throw new Exception("Invalid SQL syntax");
}
if (!BaseRepository.HasValidSqlSyntax("DELETE FROM Agents WHERE VideoSourceId = @VideoSourceId;", false,out var ex2))
{
    throw new Exception("Invalid SQL syntax");
}
